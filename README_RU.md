...
*   🧠 **LLM нормализация текста:** Опциональная пост-обработка текста через LLM с настраиваемыми параметрами генерации (temperature, top-p sampling).
...
LLM_ENABLED=true
LLM_MODEL_NAME=llm
LLM_TEMPERATURE=0.3  # Температура генерации (0.0-2.0)
...
*   `--llm-temperature`: Температура генерации LLM (0.0-2.0, по умолчанию: 0.3).
*   `--top-p`: Параметр Top-P sampling (0.0-1.0, по умолчанию: 0.9).
...
> **Смотрите также:** [📚 Подробная документация по параметрам](PARAMETERS_DOCUMENTATION_RU.md) для полной информации обо всех аргументах командной строки, параметрах конфигурации и промптах для LLM.
...
**Проблемы с LLM нормализацией**
Если LLM возвращает пустые ответы, скрипт автоматически переключается на простую нормализацию текста. Проверьте подключение к API и совместимость модели.
