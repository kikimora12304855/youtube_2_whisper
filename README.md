...
#### Устанвка uv
Этот скрипт автоматически скачивает нужный бинарный файл и добавляет его в ваш путь.
```bash
...
**Ошибка: `ffprobe/ffmpeg not found`**
Убедитесь, что FFmpeg установлен.
*   *Windows:* Скачайте сборку, распакуйте и добавьте путь к папке `bin` в переменные среды (Path).
